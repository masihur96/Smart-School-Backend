...
export class AttendanceRecordDto {
  @ApiProperty({ example: 'uuid-student-001', description: 'Student ID (UUID)' })
  @IsString()
  studentId: string;
...
export class SubmitAttendanceDto {
  @ApiProperty({ example: '2026-03-20', description: 'Attendance date (YYYY-MM-DD)' })
  @IsDate()
  @Type(() => Date)
  date: Date;

  @ApiProperty({ example: 'uuid-teacher-001', description: 'Teacher ID who is taking attendance' })
  @IsString()
  takenBy: string;

  @ApiProperty({ example: 'uuid-class-001', description: 'Class ID for which attendance is being submitted' })
  @IsString()
  classId: string;
